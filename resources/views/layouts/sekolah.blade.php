  ...
  <div>
          ...
          <li class="nav-item dropdown pe-3">
            <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
              @if (auth()->user()->photo_profile)
                <img src="{{asset(auth()->user()->photo_profile)}}" alt="Profile" class="rounded-circle object-fit-cover" style="aspect-ratio: 1/1;"/> 
              @else
            ...
            <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
              <li class="dropdown-header">
                @if (auth()->user()->photo_profile)
                  <img src="{{asset(auth()->user()->photo_profile)}}" alt="Profile" class="rounded-circle object-fit-cover my-2" style="aspect-ratio: 1/1; width: 35px;"/> 
                @else
                  <i class="ri-account-circle-fill fs-2"></i> 
                @endif
    ...
    <aside id="sidebar" class="sidebar">
      <ul class="sidebar-nav" id="sidebar-nav">
        <li class="nav-item">
            <a class="nav-link {{ ($title == 'Sekolah') ? '':'collapsed'}}" href="/sekolah"> 
                <i class="bi bi-grid"></i> 
                <span>Dashboard</span> 
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ ($title == 'Sekolah | Siswa') ? '':'collapsed'}}" href="/sekolah/siswa"> 
                <i class='bx bx-file'></i>
                <span>Pendataan Siswa SPW</span> 
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ ($title == 'Sekolah | Alumni') ? '':'collapsed'}}" href="/sekolah/alumni"> 
                <i class='bx bx-file-blank' ></i>
                <span>Pendataan Alumni</span> 
            </a>
        </li>
      </ul>
    </aside>
